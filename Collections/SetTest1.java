...
In the above program, we have created a set and added elements A, B and C to it using add method of Set interface. 
Then we have used size method of Set interface to get the number of elements in the set.
also to note that in Student class we have overriden the equals method to compare the name, course and fees of the current object with the name, course and fees of the object ref.
Then we have set itr to the iterator of the set and used hasNext and next methods of Iterator class to get the elements of the set and display their details.
Then we have created an object temp of Student class and initialized it with the details of the student.*/